...
export default function EmployerLoginsPage() {
  ...
  const load = useCallback(() => {
    ...
    fetch(`/api/employer/logins?${params.toString()}`)
      .then((r) => r.json())
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="User logins"
        description="All sign-in activity across referrers and employers"
      />
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Sign-in events will appear here when users log in."
        />
      ) : (
            ...
            <tbody className="divide-y divide-primary/5">
              {logins.map((login) => (
                <tr key={login.id} className="hover:bg-surface/50">
                  ...
                  <td className="px-4 py-3 capitalize text-muted">{login.role.toLowerCase()}</td>
